...
template=(
"""Firstly, you are a receptionist and using your Tools and function to help user. 
...
async def startup_event():
    ...
    async def info_node(state: State) -> State | Command:
        ...
        if last_ai_message is None or last_ai_message.content.strip() == "":
            ...
            # ✅ 用户发送了新消息，直接处理（Jira等工具执行后的新输入）
            print(f"[DEBUG info_node] User sent new message, processing directly")
            state["tool_images"] = []
            response = await llm_with_tools.ainvoke(state["messages"])
        else:
            # 🔍 需要用户输入，触发 interrupt
            print(f"[DEBUG info_node] Before interrupt, current tool_images count: {len(state.get('tool_images', []))}")
        ...
        # 🔧 FIX: 保留行内缩进和空格，只去除首尾空行（避免破坏Markdown表格/列表格式）
        ...
        # 🔍 调试信息：检查 info_node 中的图片提取
        print(f"[DEBUG info_node] Images from LLM response: {len(answer_images)}, Will return: {len(tool_images)} images")
    ...
    async def result_processing_node(state):
        ...
        # 🔍 调试信息：检查工具和图片提取
        print(f"[DEBUG result_processing] ===== TOOL EXECUTION DETECTED =====")
        print(f"[DEBUG result_processing] Tool name: {tool_name}")
        print(f"[DEBUG result_processing] Images extracted: {len(tool_images)}")
        print(f"[DEBUG result_processing] Strategy: {get_summary_strategy(tool_name) if tool_name else 'N/A'}")
        if tool_images:
            print(f"[DEBUG result_processing] First image keys: {list(tool_images[0].keys()) if tool_images else 'N/A'}")
        print(f"[DEBUG result_processing] ========================================")
        ...
        if summary_strategy == "none":
            ...
            # ✅ 工具处理完成，返回状态（路由由条件边决定）
            print(f"[DEBUG result_processing] ===== CHART TOOL COMPLETED =====")
            print(f"[DEBUG result_processing] Tool name: {tool_name}")
            print(f"[DEBUG result_processing] Tool images count: {len(tool_images)}")
            print(f"[DEBUG result_processing] First image preview: {tool_images[0] if tool_images else 'No images'}")
            print(f"[DEBUG result_processing] AI_Response: {state['AI_Response'][:100]}")
            print(f"[DEBUG result_processing] ========================================")
            return {
                "messages": state["messages"],
                "Thinking": state["Thinking"],
    ...
    # ✅ 添加条件路由函数：工具执行完成后直接结束，不回到 info_node
    def route_after_processing(state: State) -> Literal["__end__"]:
        # 所有工具执行完成后都直接END，避免触发不必要的interrupt
        tool_images_count = len(state.get("tool_images", []))
        ai_response = state.get("AI_Response", "")[:100]  # 只打印前100字符
        ...
        print(f"[DEBUG route_after_processing] - AI_Response preview: {ai_response}")
        print(f"[DEBUG route_after_processing] - Full tool_images: {state.get('tool_images', [])[:1]}")  # 只打印第一个
        return END
...
async def execute_2(plan: dict):
    ...
    # 🔧 FIX: 从最新的 result 中获取 tool_images，而不是从找到 AI_Response 的那个
    print(f"[DEBUG execute_2] ===== SEARCHING FOR IMAGES =====")
    print(f"[DEBUG execute_2] Total results count: {len(results)}")
    for idx, result in enumerate(reversed(results)):
        tool_images_value = find_value(result, "tool_images")
        print(f"[DEBUG execute_2] Result [{len(results)-idx-1}]: tool_images = {len(tool_images_value) if tool_images_value else 'None'}")
        if tool_images_value is not None:
            dynamic_vars["tool_images"] = tool_images_value
            print(f"[DEBUG execute_2] ✅ Found tool_images in result [{len(results)-idx-1}], count: {len(tool_images_value)}")
            break
    ...
    # 🔍 调试信息：检查最终返回的图片数量
    print(f"[DEBUG execute_2] ===== FINAL RESPONSE TO FRONTEND =====")
    print(f"[DEBUG execute_2] Images count: {len(response_payload['tool_images'])}")
    print(f"[DEBUG execute_2] AI_Response: {response_payload['AI_Response'][:100]}")
    print(f"[DEBUG execute_2] ===========================================")
