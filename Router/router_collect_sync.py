...
import time
from AD_subgraph2 import build_subgraph
...
async def startup_event():
    ...
    print("building subgraph", type(subgraph))
...
async def execute_2(plan: dict):
    ...
    if first_run:
        ...
        print("---------------snapshot3")
        print(current_values["messages"])
        print("AI_Response")
        print(current_values["AI_Response"])
        print("Thinking")
        print(current_values["Thinking"])
        print("---------------subpshot")
        sub_snapshot = subgraph.get_state(config={"configurable": {"thread_id": thread_id}})
        print(sub_snapshot.values)
    #results = []
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("router_collect_sync:app", host="0.0.0.0", port=777, reload=False)
