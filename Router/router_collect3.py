...
def get_state(state: State) -> Literal["human_window", "subgraph_node", "info", "__end__","tools"]:
    ...
    if isinstance(messages[-1], AIMessage) and messages[-1].tool_calls:
        ...
        Approved_tools = ["get_latest_UP_version", "bbb", "ccc"]
...
def get_state_sub(state: State) -> Literal["human_approve_sub", "call_subgraph_sec","info"]:
    ...
    subgraph_status = state["subgraph_status"]
    print("subgraph_status in get_state_sub ",subgraph_status)

    if human_status:
        print("human_status is",human_status)
        return "human_approve_sub"
    if subgraph_status == "Finish" or subgraph_status == "done":
        return "info"
    else:
...
def human_approve_sub(state: State):
    ...
    resume_data  = interrupt(
                {
                    "question": "Is this correct in human approve?",
                    # Surface the output that should be
                    # reviewed and approved by the human.
                }
            )
    resp = subgraph.run(session_id=session_id, input_data=resume_data)
    print("session id is in human_approve_sub ",session_id)
    print("resp is in human_approve_sub ",resp)
    node_result = next(iter(resp.values()))
    ai_response = node_result.get("AI_Response", "")
    human_in_the_loop = node_result.get("human_in_the_loop", "")
    status = node_result.get("status", "")
    state["Thinking"]=""
    state["AI_Response"]=ai_response
    state["human_in_the_loop"]=human_in_the_loop
    state["subgraph"]=""
    state["subgraph_status"]=status
    print("[debug]state in human_approve_sub", state)
    ...
    #human_in_the_loop = interrupt_info.value.get("human_in_the_loop", "")
    #print("ai _message in human_app sub,",ai_message)
    ...
    return  Command(goto="call_subgraph_sec")
...
def human_approval(state: State) -> Command[Literal["tools", "info"]]:
    ...
    if ai_message.tool_calls:
        ...
        llm_output = "\n".join([f"Please approve the execution of function : {c['name']} using ({c['args']})" for c in calls])
        ''''llm_output = "```markdown\n" + "\n".join(
    [f"Please approve the execution of function : {c['name']} using ({c['args']})" for c in calls]
) + "\n```"'''
    else:
...
async def startup_event():
    ...
    async def call_subgraph(state: State) -> State:
        ...
        while True:
            if "__interrupt__" in resp:
                ...
                ai_message = interrupt_info.value.get("AI_Response", "需要用户输入")
                human_in_the_loop = interrupt_info.value.get("human_in_the_loop", "")
                #value = interrupt({"AI_Response": ai_message})
    ...
    async def call_subgraph_sec(state: State) -> State:
        user_input: Dict[str, Any] = {}
        session_id = state["session_id"]
        print("Trying to call subgraph sec")
        value = interrupt({"Interrupt in call_subgraph_sec" })
        resp = subgraph.run(session_id=session_id, input_data={"text": value})
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("router_collect3:app", host="0.0.0.0", port=777, reload=False)
