...
template1=('''你是一个智能信息收集 Agent，任务是根据用户需求收集信息，并整理为 JSON 格式返回。你有以下能力：
...
def get_state_collect(state: Collect_State, config: RunnableConfig) -> Literal["human_approval", "sub_info", "__end__","json_gen_node"]:
    messages = state["messages"]
    thread_id = config.get("configurable", {}).get("thread_id", None)
    print(f"[debug] info_node thread_id: {thread_id}")
...
def human_approval(state: Collect_State) -> Command[Literal["jenkins", "sub_info"]]:
...
async def build_subgraph():
    ...
    async def use_tools(state: Collect_State) -> Collect_State | Command[Literal["__end__"]]:
        ...
        return state
    async def info_node(state: Collect_State, config: RunnableConfig) -> Collect_State | Command:
        global global_log
        ...
        if not any(isinstance(m, SystemMessage) for m in state["messages"]):
            ...
            messages.insert(0, SystemMessage(content=template1))
        ...
        # ✅ 第一次调用：没有 AI 响应，直接调 LLM
            print("fist infor in sub")
            response = await llm.ainvoke(state["messages"])
        else:
            print("sec infor in sub")
        # ✅ 第二次调用：已有 AI 响应 → 打断点等待用户输入
            value = interrupt({
                "text_to_revise": "interrupt in sub infor"
            })
            #if not any(isinstance(m, SystemMessage) for m in state["messages"]):
                #state["messages"].insert(0, SystemMessage(content=final_template))
            ...
            messages.append(HumanMessage(content=str(value)))
...
"""
    ...
    graph_builder.add_node("sub_info", info_node)
    ...
    #graph_builder.set_output_node("sub_info") 

    graph_builder.add_edge(START, "sub_info")
    graph_builder.add_conditional_edges("sub_info", get_state_collect)
    ...
    graph_builder.add_edge("jenkins",END)
