...
1) If a tool needs to be called:  
   - You MUST list all required parameters and their values **inside the [FinalAnswer] section and made it readable**.  
...
async def startup_event():
    ...
    conn = sqlite3.connect("langgraph_state.sqlite", check_same_thread=False)

    #memory = InMemorySaver()
    conn = await aiosqlite.connect(
        "langgraph_state.sqlite",
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("router_7_app:app", host="0.0.0.0", port=777, reload=True)
