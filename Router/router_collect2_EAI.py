...
def get_state(state: State) -> Literal["human_window", "subgraph_node", "info", "__end__","tools"]:
    ...
    if isinstance(messages[-1], AIMessage) and messages[-1].tool_calls:
        ...
        Approved_tools = ["get_latest_UP_version", "check_UP_number", "DU_Name_IP_Mapping","extract_rrc_msgs","query_jira_issues","get_issue_statistics","generate_line_chart"]
...
async def startup_event():
    ...
    async def info_node(state: State) -> State | Command:
        ...
        print(f"⚠️ Calling Eric LLM")
        llm = await create_llm()
        try:
            print("Health check of the LLM")
            response = await llm.ainvoke("ping")
        except Exception as e:
            print(f"⚠️ Calling Eric LLM and meet issue : {e}")
        ...
        #state["Thinking"]=state["Thinking"]+reasoning
        if reasoning:
    ...
    async def result_processing_node(state):
        ...
        latest_tool_message=latest_tool_message.content
        latest_tool_message_output = f"### 🛠️ Tool Execution Result\n```json\n{latest_tool_message}\n```"
        print("latest_tool_message in result_processing_node is",latest_tool_message)
        analysis_message = {
            "role": "user", 
        ...
        state["Thinking"]=state["Thinking"]+latest_tool_message_output
        state["AI_Response"]=final_answer
        ...
        for idx, item in enumerate(results, start=1):
            ...
            for target in reversed(results):
                ...
                if ai_response:  # 找到就停止遍历
                    #print("target inCurrent results ",target)
                    thinking = find_value(target, "Thinking")
                    #print("Thinking inCurrent results 1",thinking)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("router_collect2_EAI:app", host="0.0.0.0", port=878, reload=True)
