...
def get_state(state: State) -> Literal["human_window", "subgraph_node", "info", "__end__"]:
...
async def startup_event():
    ...
    async def call_subgraph(state: State) -> State:
        last_msg = state["messages"][-5].content if state["messages"] else ""
        print(f"[debug]last message:{last_msg}")
        final_response = None
        async for event in subgraph.astream_events(
        {
            "messages": last_msg,
            "AI_Response": "",
            "Thinking": "",
            ...
            if etype == "on_chain_end":
                ...
                print(f"[debug] subgraph final response: {final_response}")
                print("111111")
                if isinstance(final_response, dict):
                    print("final_response is dic")
                    new_state: Collect_State = {
                        "messages": state["messages"]+final_response.get("messages", []),
                        "AI_Response": final_response.get("AI_Response", ""),
                        "Thinking": final_response.get("Thinking", ""),
                        "human_in_the_loop": final_response.get("human_in_the_loop", ""),
                        "json_result": final_response.get("json_result", {}),
                        "session_id": final_response.get("session_id", state.get("session_id", ""))
                    }
                    print(f"[debug] subgraph final new_state: {new_state}")
                    return new_state
    ...
    async def call_subgraph_sec(state: State) -> State:
        ...
        async for event in subgraph.astream_events(
        {
            "messages": state["messages"] + [human_input],
            "AI_Response": "",
            "Thinking": "",
            "human_in_the_loop": "",
            "json_result": {},
            ...
            if etype == "on_chain_end":
                ...
                if isinstance(final_response, dict):
                    ...
                    print(f"[debug] subgraph final new_state in sec: {new_state}")
                    return new_state
    ...
    graph_builder.add_edge("call_subgraph_sec", "call_subgraph_sec")
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("router_collect:app", host="0.0.0.0", port=888, reload=False)
