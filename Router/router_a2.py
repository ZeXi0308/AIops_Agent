...
memoryy={"message":"unmodified message",
            "action":"text",
            "task_id":"1"}
...
def check_node(state: OverallState) ->CompletedState:
    ...
    if state.get("hardware_name",None) is not None and not state["hardware_name"].startswith(tuple(valid_DU_names)):
        #print(f"[debug]invalid hardware name:{state["hardware_name"]}")
    ...
    if state["UP_version"] is None:
        ...
        if state["UP_version"].startswith("latest UP") or state["UP_version"].startswith("latest up"):
            ...
            state_dict["UP_version_lookup"]=True
        if not (state["UP_version"].startswith("CXC") or state["UP_version"].startswith("cxc")) :
...
def language_detect():
    return
def look_up_du_ip_pair(state: OverallState)-> OverallState:
    #TODO:tool call
    if state["hardware_name"] is None:
        print("[debug]DU_name is None")
        state["hardware_name"]="CNBJITDUS_default"
    else:
        print("[debug]ip is None")
        state["IP"]="111.111.111.111"
    return state
def final_confirmation(state: OverallState)-> Command[Literal["execution_node","modify_configuration"]]:
    ...
    if state["operation"] in ["install","INSTALL","Install"] and not state["Configuration"]:
    ...
    memoryy={"message":memoryy_str,
            "action":"HITL",
            "task_id":"1"}
    print("[debug]final confirm")   
    print(f"[debug]memory message:{memoryy['message']}")
...
def latest_up(state: OverallState)-> OverallState:
    print("[debug]get the latest up version: banana version")
    state["UP_version"]="CXC_latest"
    return state
def check_up_number(state:CompletedState)->CompletedState:
    #TODO:tool call
    number=random.randint(10)
    print(f"[debug]:checked up number, got {number}")
    state["up_count"]=number
    return state
...
def execution_node(state: OverallState):
    global subgraph_processing
    subgraph_processing=False
    global memoryy
    print("[debug]I am executing this job.")
    memoryy_str="[debug]I am executing this job."
    print("Command executing, parameters shown as below:\n")
    ...
    api_token="api_token"
    crumb="crumb"
    DU_name=state["DU_name"]
    UP_version=state["UP_version"]
    operation=state["operation"]
    scenario_for_netconf="aaa"
    scenario_for_tags="aaa"
    tags=state["Tags"]
    post_step=state["Post_step"]
...
async def startup_event():
    ...
    async def input_node(state: State) -> OverallState:
        ...
        set_verbose(True)
        set_debug(True)
        last_msg = state["messages"][-3].content if state["messages"] else ""
        ...
        **Literally just return the json itself without any suffix or prefix.**
        """
        content=prompt_hint+"user input:"+last_msg
        print(f"[debug]last message:{last_msg}")
    ...
    async def modify_configuration(state: OverallState) -> OverallState:
        ...
        memoryy_str=memoryy_str+"how would you like to modify:"
        memoryy={"message":memoryy_str,
                "action":"text",
                "task_id":"1"}
        print(f"[debug]memory message:{memoryy['message']}")
        ...
        try:
            prompt_hint= """Your task is to gather specific information from the user regarding DU(a kind of device) install, upgrade, or configuration.
                ...
                **Literally just return the json itself without any suffix or prefix.**
                """
            prompt=prompt_hint+"user input:"+value
            response_str=llm.invoke(prompt)
            response_dict = json.loads(response_str.model_dump_json())
        except json.JSONDecodeError:
            # 处理无效JSON的情况
            response_dict = {}
        #print("[debug]modify response="+json.dumps(response_dict))
        response_content=response_dict["content"]
        response=json.loads(extract_json_content(response_content))
        #print("[debug]modify response="+json.dumps(response))
        result_dict={}
        for key in state:
    ...
    async def makeup_node(state: OverallState) -> OverallState:
        ...
        # 一种可行的设计是在上一步的check里面把不对的参数全部置none。
    ...
    graph_builder.add_node("language_detect",language_detect)
    ...
    #graph_builder.add_edge("subgraph_node","receive_node")
    #graph_builder.add_edge("receive_node", "input_node")
    ...
    #builder.add_edge("final_confirmation", "execution_node")
    graph_builder.add_edge("rewrite_sc", "final_confirmation")
    graph_builder.add_edge("latest_up", "check_node")
    #builder.add_edge("modify_configuration","final_confirmation")
    ...
    #builder.add_conditional_edges("final_confirmation",should_execute,)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("router_a2:app", host="0.0.0.0", port=7777, reload=True)
