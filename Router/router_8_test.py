...
def check_node(state: OverallState) ->CompletedState:
    ...
    try:
        if state.get('operation',None)==None:
            state_dict["not_completed"]=True
        elif state["operation"]=="upgared":
            state["operation"]="upgrade"
        elif state["operation"] not in valid_operations:
        ...
        if state["UP_version"] is None:
            ...
            if state["UP_version"].startswith("latest"):
        ...
        valid_post_steps = ["NULL",
                            "initial_configuration",
                            "CUCP_CUUP_DU_5Qi_table",
                            "iodt_recomm_netconfig",
                            "VONR_Configuration"]
        if state["Post_step"] is None or state["Post_step"] not in valid_post_steps:
            state["Post_step"]=None
            state_dict["not_completed"]=True
            print(f"[debug]invalid Post_step:{state['Post_step']}")
            
        
        valid_senario_for_tags=["NULL",
                                "nr_scenario_1LB_C1_plus_2MB_C2_Radio_3cell_1F2T",
        ...
        if state["Scenario_for_tags"] is None or state["Scenario_for_tags"] not in valid_senario_for_tags:
            state["Scenario_for_tags"]=None
            state_dict["not_completed"]=True
            print(f"[debug]invalid Scenario for tags:{state['Scenario_for_tags']}")
        ...
        if state["Netconf"] is None or state["Netconf"] not in valid_netconf:
            state["Netconf"]=None
            state_dict["not_completed"]=True
        ...
        if state["operation"] in ["configuration","Configuration","CONFIGURATION"]\
            and (state.get("Tags",None) is None or state.get("Tags",None)=="NULL")\
                and (state.get("Scenario_for_tags",None) is None or state.get("Scenario_for_Tags",None)=="NULL"):
...
def final_confirmation(state: OverallState)-> Command[Literal["execution_node","modify_configuration"]]:
    ...
    global_log["AI_Response"].append(state.get("recommendation",""))
    del global_log["Human in the Loop"][:]
...
async def execution_node(state: OverallState):
    ...
    subgraph_processing=False
    memoryy_str="Command executing, parameters shown as below:\n"
    for key in state:
        if key != "recommendation":
            value = state.get(key) or ""  
            memoryy_str += f"{key}:{value}\n"
    ...
    try:
        ...
        time.sleep(10)
        print("[debug]command 2 in execution node:"+ cmd_str)
...
async def startup_event():
    ...
    async def input_node(state: State) -> OverallState:
        global global_log
        print("[debug]Global log in input node is,",global_log)
        del global_log["AI_Response"][:]
        print("[debug]After deleting,Global log in input node is,",global_log)
        global subgraph_processing
        ...
        count = len(human_messages)
        if count == 0:
            return  
        elif count == 1:
            user_input = human_messages[0].content 
        else:
            user_input = human_messages[-2].content + "\n" + human_messages[-1].content
        ...
        8. recommendation: a string in natural language. it can be a few sentences hinting the user to make up all missing elements by providing the corresponding lists.Provide all lists mentioned above for users to choose from.
            for example:
                'you did not provide Tags, Netconf,post_step. \n
                Netconf can be choosed from [nr_scenario_1LB_C1_plus_2MB_C2_Radio_3cell_1F2T,nr_scenario_HB_8cell_TDD,nr_scenario_LB_3cell_FDD,nr_scenario_MB_AIR_AAS_C2_Radio_3cell_TDD,
                nr_scenario_MB_RUS_C1_Radio_3cell_TDD,nr_scenario_MultiSec_MB_RUS_C1_3Cells_TDD,scenario_lte_FDDESS_scenario_ESS_3cell_FDD], 
                while post_steps can be choose from [initial_configuration, CUCP_CUUP_DU_5Qi_table,iodt_recomm_netconfig,VONR_Configuration].'
            **For this field, if you have nothing to say, at least put an empty string.**

        Return only with the json format shown below. If unsure,** put a json null ** in the corresponfing field.
        ** If the user explicitly states that a field should be empty, put a string "NULL" in the corresponding field.**
        {operation:str
        hardware_name:str
        IP:str
        UP_version:str
        ...
        **Literally just return the json itself without any suffix or prefix.**
        """
        content=prompt_hint+"user input:"+user_input
        #response_str=llm.invoke(content)
        response_str=await llm.ainvoke(content)
        ...
        rec=response.get("recommendation",None)
        return {
        ...
        try:
            prompt_hint= """Your task is to gather specific information from the user regarding DU(a kind of device) install, upgrade, or configuration.
                ...
                ** if the user mentions specific configuration, please put it into the field of "Configuration". It has nothing to do with the operation.**
                ...
                8. recommendation: a string in natural language. it can be a few sentences hinting the user to modoify elements.Provide all lists mentioned above for users to choose from.
                    for example:
                        'Netconf can be choosed from [nr_scenario_1LB_C1_plus_2MB_C2_Radio_3cell_1F2T,nr_scenario_HB_8cell_TDD,nr_scenario_LB_3cell_FDD,nr_scenario_MB_AIR_AAS_C2_Radio_3cell_TDD,
                        nr_scenario_MB_RUS_C1_Radio_3cell_TDD,nr_scenario_MultiSec_MB_RUS_C1_3Cells_TDD,scenario_lte_FDDESS_scenario_ESS_3cell_FDD], 
                        while post_steps can be choose from [initial_configuration, CUCP_CUUP_DU_5Qi_table,iodt_recomm_netconfig,VONR_Configuration].'
                ...
                ** ONLY IF the user explicitly states that a field should be empty, put a string "NULL" in the corresponding field.**
                Return only with the json format shown below. If unsure,** put a json null ** in the corresponfing field.
                {operation:str
                hardware_name:str
                IP:str
            ...
            response_str=await llm.ainvoke(prompt)
            response_dict = json.loads(response_str.model_dump_json())
        except json.JSONDecodeError:
            response_dict = {}
        response_content=response_dict["content"]
    ...
    async def look_up_du_ip_pair(state: OverallState)-> OverallState:
        global global_log
        #del global_log["AI_Response"][:]
        if state["hardware_name"] is None or state["hardware_name"]=="NULL":
    ...
    async def makeup_node(state: OverallState) -> OverallState:
        ...
        for key in state:
            if key != "recommendation":
                value = state.get(key) or ""  
                memoryy_str += f"{key}:{value}\n"
        memoryy_str=memoryy_str+"the following elements are missing or incorrect,please make up:"
        for key in state:
        ...
        for key in state:
            if state.get(key,None) ==None or state.get(key,None) =="NULL":
        ...
        try:
            ...
            1. The operation the user wants to perform. Currently, three operations are supported: install, upgrade, and configuration.
            ** put a json null in this field if the user did not mention.**
            2. The device name or IP address. The name usually starts with "cnbj" or "seki".
            put the device name into the field of "hardware_name".
        ...
        for key in state:
            if not state.get(key,None)==None:
                ...
                result_dict[key]=response.get(key,None)
        result_dict["recommendation"]=response.get('recommendation',None)
        print(f"[debug]the full dictionary:{result_dict}")
        return result_dict
...
async def execute_2(plan: dict):
    ...
    print("global_log to front end is", global_log,thread_id)
    return {
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("router_8_test:app", host="0.0.0.0", port=777, reload=False)
