...
async def startup_event():
    ...
    async def jenkins_excute_command(state:Collect_State) :
        import time
    ...
    async def info_node(state: Collect_State) -> Collect_State | Command:
        ...
        print("[debug]AI response in info node", response)
        return {
    ...
    async def json_gen_node(state:Collect_State) -> Collect_State:
        ...
        prompt_text = f"""你是一个严格的JSON生成器。用户提供的信息收集完毕后，你只需要输出符合JSON Schema的最终结果，不要额外解释。
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("collect_infor_test:app", host="0.0.0.0", port=7107, reload=True)