...
- [FinalAnswer]  ← **mandatory**, must contain meaningful text (at least one sentence)and using Markdown Format!
...
def get_state_sub(state: State) -> Literal["human_approve_sub", "call_subgraph_sec"]:
...
def human_approve_sub(state: State):
    ...
    if resume_type == "accept":   
        ...
        try:
            ...
            state["AI_Response"]=ai_response
            print("[debug]state in info node", state)
            return Command(update={"AI_Response": state["AI_Response"], "Thinking": state["Thinking"], "subgraph": ""},   goto="info")
        except Exception as e:
            print("请求出错:", e)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("router_collect2:app", host="0.0.0.0", port=878, reload=False)
