...
2. hardware_name： The name usually starts with "cnbj" or "seki" or a IP address.put the device name into the field of "hardware_name".  This item is mandatory 
...
async def startup_event():
    ...
    async def jenkins_excute_command(state: Collect_State) -> Collect_State:
        ...
        try:
            ...
            match = re.search(r'/(\d+)/?$', url_ret_str)
            if match:
                number = match.group(1)
                new_url = f"http://iodt.gic.ericsson.se:8443/blue/organizations/jenkins/Test_Display/detail/Test_Display/{number}/pipeline"
                print(new_url)
            else:
                new_url=url_ret_str
            print("url is", new_url)
            state["AI_Response"]=new_url
            #final_result = f"\nurl: {url_ret_str}"
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("AD_graph_EAI:app", host="0.0.0.0", port=7107, reload=True)
