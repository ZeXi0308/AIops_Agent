...
session_logger = SessionLogger("Kunlun_Agent")
...
2) Special case: DU or Baseband (GNB/ENB) installation, configuration, or upgrade,or Apply IODT recommended parameters
   ...
   - If the user mentions "apply IODT recommended parameters", it should be considered as this scenario.
...
def get_state(state: State) -> Literal["human_window", "subgraph_node", "info", "__end__","tools"]:
    ...
    if state.get("subgraph") in ["true", "Trace_Agent"]:
        ...
        session_logger.log(session_id, "INFO", "get_state", "Result", "subgraph_node")
        return "subgraph_node"
    if not messages:
        session_logger.log(session_id, "INFO", "get_state", "Result", "info")
        return "info"
    if isinstance(messages[-1], AIMessage) and messages[-1].tool_calls:
        ...
        if any(_is_auto_tool(name) for name in tool_names):
            ...
            session_logger.log(session_id, "INFO", "get_state", "Result", "execute without HIL")
            return "tools"
        else:
            session_logger.log(session_id, "INFO", "get_state", "Result", "execute with HIL")
            return "human_window"
...
def get_state_sub(state: State) -> Literal["human_approve_sub", "call_subgraph_sec","info"]:
    ...
    if human_status:
        ...
        session_logger.log(session_id, "INFO", "get_state_sub", "Result", "human_approve_sub")
        return "human_approve_sub"

    if isinstance(ai_message, str) and any(keyword in ai_message for keyword in keywords):
        session_logger.log(session_id, "INFO", "get_state_sub", "Result", "info")
        return "info"
    else:
        session_logger.log(session_id, "INFO", "get_state_sub", "Result", "call_subgraph_sec")
...
def human_approve_sub(state: State):
    ...
    session_logger.log(session_id, "INFO", "human_approve_sub", "Prepare to call", f"{subgraph_value}:{port}")
...
async def startup_event():
    ...
    async def info_node(state: State) -> State | Command:
        ...
        # ============================================================
        # 🔧 Initialize LLM first (needed for orchestration)
        # ============================================================
        ...
        # ============================================================
        # 🎭 MCP Multi-Server Orchestration Detection
        # ============================================================
        human_msgs = [m for m in state["messages"] if isinstance(m, HumanMessage)]
    ...
    async def result_processing_node(state):
        ...
        session_logger.log(session_id, "INFO", "result_processing_node", "start", "Start result_processing_node")
        # 不继承旧图，直接从零开始
        ...
        try:
            print("Health check of the LLM")
            response = await llm.ainvoke("ping")
            session_logger.log(session_id, "DEBUG", "result_processing_node", "llm_health_check", "Using GPT")
        except Exception as e:
            llm = await create_llm()
            session_logger.log(session_id, "WARNING", "result_processing_node", "llm_fallback", "Using ERICAI", error_info=str(e))
        ...
        session_logger.log(session_id, "INFO", "result_processing_node", "Finish", "Finish result_processing_node")
        return {
...
async def execute_2(plan: dict):
    ...
    # 从最新的 result 中获取 tool_images，只选择非空的图片列表
    for result in reversed(results):
        tool_images_value = find_value(result, "tool_images")
        if tool_images_value and len(tool_images_value) > 0:
