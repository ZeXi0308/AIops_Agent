...
async def startup_event():
    ...
    async def info_node(state: State) -> State | Command:
        ...
        # 🔧 不在这里清空图片！图片清空由 result_processing_node 和 /run_test/ 端点控制
        # info_node 只负责保持现有图片或添加新图片（如果 LLM 回复中有）
        ...
        # 检查最近几条消息，判断是否刚从 result_processing_node 回来
        recent_messages = messages[-5:] if len(messages) >= 5 else messages
        has_tool_result = any(isinstance(m, ToolMessage) for m in recent_messages)
        
        # 只在以下情况才需要 interrupt：
        # 1. 最后一条是 AIMessage（纯对话回复）
        # 2. 没有 tool_calls（不是工具调用）
        # 3. 不是刚从工具执行流程回来（如果有 ToolMessage，说明是工具流程）
        should_interrupt = (
            isinstance(last_message, AIMessage) and
            not getattr(last_message, 'tool_calls', None) and
            not has_tool_result  # ← 关键：如果有工具结果，不 interrupt
        )
        
        if should_interrupt:
            # 等待用户继续对话
            print("🔔 [info_node] Interrupt: waiting for user to continue conversation")
            ...
            # 直接处理
            print(f"🚀 [info_node] No interrupt (last_msg: {type(last_message).__name__}, has_tool_result: {has_tool_result})")
            response = await llm_with_tools.ainvoke(state["messages"])
        ...
        # 🔧 正确逻辑：保持现有图片，如果 LLM 回复中有图片则追加
        existing_images = state.get("tool_images", [])
        if answer_images:
            tool_images = existing_images + answer_images
            print(f"📝 [info_node] LLM generated {len(answer_images)} images, total: {len(tool_images)}")
        else:
            tool_images = existing_images
            print(f"📝 [info_node] No new images from LLM, keeping {len(tool_images)} existing images")
    ...
    async def result_processing_node(state):
        ...
        # 🔧 BUG FIX: 在处理新工具结果前清空旧图片（这是正确的清空位置）
        state["tool_images"] = []
        ...
        def _prepare_tool_payload(msg: ToolMessage | None) -> tuple[str, list[dict[str, str]], str | None]:
            ...
            # 🔍 详细调试日志
            print(f"🔍 [_prepare_tool_payload] Tool name: {tool_name}")
            print(f"🔍 [_prepare_tool_payload] Raw content type: {type(raw_content)}")
            print(f"🔍 [_prepare_tool_payload] Raw content preview (first 200 chars): {str(raw_content)[:200]}")
            
            cleaned_text, images, _ = process_tool_message(tool_name, raw_content)
            
            print(f"🔍 [_prepare_tool_payload] After process_tool_message: cleaned_text length={len(cleaned_text)}, images count={len(images)}")
        ...
        print(f"🖼️ [result_processing] Extracted {len(tool_images)} images from tool '{tool_name}'")
        ...
        print(f"🔍 [result_processing] Tool '{tool_name}' → summary_strategy: '{summary_strategy}'")

        if summary_strategy == "none":
            print(f"✅ [result_processing] Skipping LLM summary for tool '{tool_name}', returning chart directly")

            # 对于图表工具，不需要显示详细的 JSON，只需简洁提示
            if tool_images:
                final_answer_to_log = "✅ Chart has been successfully generated."
            else:
                final_answer_to_log = (
        ...
        # 注意：LLM 不会回复图片，answer_images 应该总是空的
        # 如果有图片，也只是用于调试，实际不应该出现
...
async def execute_2(plan: dict):
    ...
    if first_run:
        ...
        print("🔧 [/run_test/] First run, tool_images initialized to []")
    else:
        print("run into second ")
        # 🔧 续话：不清空图片（让 result_processing_node 决定）
        # 如果用户在 interrupt 后输入的是新请求，图片会在下一次工具调用时被清空
    ...
    print(f"📤 [/run_test/] Returning {len(dynamic_vars.get('tool_images', []))} images to frontend")
    response_payload = {
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main_zhenyu:app", host="0.0.0.0", port=878, reload=True)