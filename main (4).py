...
async def startup_event():
    ...
    async def result_processing_node(state):
        ...
        print(f"[DEBUG result_processing] Found tool message: {latest_tool_message is not None}")
        ...
        print(f"[DEBUG result_processing] Tool: {tool_name}, Images extracted: {len(tool_images)}")
        if tool_images:
            print(f"[DEBUG result_processing] First image preview: mime={tool_images[0].get('mime', 'N/A')}, data_length={len(tool_images[0].get('data', ''))}")
        ...
        if summary_strategy == "none":
            ...
            print(f"[DEBUG result_processing] Returning (strategy=none) with {len(tool_images)} images\n")
            return {
                "messages": state["messages"],
                "Thinking": state["Thinking"],
                "AI_Response": state["AI_Response"],
        ...
        print(f"[DEBUG result_processing] Returning (with LLM summary) with {len(tool_images)} images\n")
        return {
    ...
    # ✅ 条件路由函数：工具执行完成后直接END，不回到info_node
    def route_after_processing(state: State) -> Literal["__end__"]:
        ...
        print(f"[DEBUG route] tool_images count: {tool_images_count}")
        print(f"[DEBUG route] Returning END to complete flow")
        print(f"[DEBUG route] ==========================================\n")
        return END
    ...
    # ✅ 工具执行后直接 END（避免回到 info 触发不必要的 interrupt）
...
async def execute_2(plan: dict):
    ...
    if first_run:
        ...
        # 🔧 FIX: 检查图状态，区分 interrupt resume 和新消息
        current_state = graph.get_state(config)
        
        # 如果有 next 节点等待（表示有 interrupt），用 resume
        if current_state and current_state.next:
            print(f"[DEBUG] Resuming interrupt, next nodes: {current_state.next}")
            command = Command(resume=user_input)
            stream = graph.astream(command, config=config, stream_mode="values")
        else:
            # 图已结束，这是新的对话轮次，追加新消息
            print("[DEBUG] Graph ended, appending new message")
            new_message = HumanMessage(content=user_input)
            update_state = {"messages": [new_message]}
    ...
    # Default empty values to avoid KeyError when no graph outputs arrive
    ...
    print(f"[DEBUG execute_2] Total results: {len(results)}")
    for idx, result in enumerate(reversed(results)):
        tool_images_value = find_value(result, "tool_images")
        imgs_count = len(tool_images_value) if tool_images_value else 0
        print(f"[DEBUG execute_2] Result[{len(results)-idx-1}]: tool_images={imgs_count}")
        if tool_images_value is not None:
            dynamic_vars["tool_images"] = tool_images_value
            print(f"[DEBUG execute_2] ✅ Selected result[{len(results)-idx-1}] with {imgs_count} images")
            break
    ...
    print(f"\n[DEBUG execute_2] ===== FINAL RESPONSE =====")
    print(f"[DEBUG execute_2] Returning {len(response_payload['tool_images'])} images to frontend")
    print(f"[DEBUG execute_2] AI_Response: {len(response_payload['AI_Response'])} chars")
